...
class ProfileViewController: UIViewController {
    ...
    private func setupUI() {
        ...
        stockButton.setTitle("View Profile Details", for: .normal)
        stockButton.backgroundColor = .systemBlue
        stockButton.setTitleColor(.white, for: .normal)
        stockButton.layer.cornerRadius = 8
        stockButton.addTarget(self, action: #selector(showProfileDetail), for: .touchUpInside)
    ...
    @objc private func showProfileDetail() {
        viewModel.showProfileDetail()
    }
}
