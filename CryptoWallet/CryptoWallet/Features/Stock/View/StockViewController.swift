...
class StockViewController: UIViewController {
    ...
    private func setupUI() {
        ...
        stockButton.setTitle("View Apple Stock Details", for: .normal)
        stockButton.backgroundColor = .systemBlue
        stockButton.setTitleColor(.white, for: .normal)
        stockButton.layer.cornerRadius = 8
        stockButton.addTarget(self, action: #selector(showStockDetail), for: .touchUpInside)
    ...
    @objc private func showStockDetail() {
        viewModel.showStockDetail()
    }
}
