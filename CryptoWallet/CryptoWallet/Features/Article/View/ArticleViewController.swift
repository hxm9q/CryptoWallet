...
class ArticleViewController: UIViewController {
    ...
    private func setupUI() {
        ...
        stockButton.setTitle("View Article Details", for: .normal)
        stockButton.backgroundColor = .systemBlue
        stockButton.setTitleColor(.white, for: .normal)
        stockButton.layer.cornerRadius = 8
        stockButton.addTarget(self, action: #selector(showArticleDetail), for: .touchUpInside)
    ...
    @objc private func showArticleDetail() {
        viewModel.showArticleDetail()
    }
}
